...
from .celery import send_activation_code
...
class SignupSerializer(serializers.ModelSerializer):
    ...
    def validate(self, validated_data):
        ...
        if password != password_confirm:
            raise serializers.ValidationError('Passwords do not match! It must to be the same!')
...
class LoginSerializer(serializers.Serializer):
    ...
    def validate(self, attrs):
        ...
        if email and password:
            ...
            if not user:
                message = 'Provided credentials do not allow to log in'
                raise serializers.ValidationError(message, code='authorization')
        else:
            message = '"email" & "password" are mandatory fields .'
            raise serializers.ValidationError(message, code='authorization')
...
class CreateNewPasswordSerializer(serializers.Serializer):
    ...
    def validate_activation_code(self, code):
        if not MyUser.objects.filter(activation_code=code, is_active=False).exists():
            raise serializers.ValidationError('Provided confirmation code is incorrect')
        return code
    ...
    def save(self, **kwargs):
        ...
        try:
            user = MyUser.objects.filter(email=email, activation_code=code, is_active=False)[0]
            print(user)
